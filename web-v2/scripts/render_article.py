...
LINE_ENDINGS = set("，。！？；,.!;?")
RENAME_RETRY_DELAYS = (0.1, 0.25, 0.5, 1.0, 2.0)
...
def rewrite_character_limit(source_character_count: int) -> int | None:
    if source_character_count == 0:
        # Image-only sources have no textual baseline, so there is no ratio target.
        return None
    return math.ceil(source_character_count * 0.70)
...
def next_package_dir_after_conflict(
    result_dir: Path, title: str, now: datetime, attempted: set[str]
) -> Path:
    """Return a fresh fallback name after a concurrent commit collision."""
    prefix = f"{now:%m%d}-{sanitize_filename(title)}-{now:%H%M%S}"
    counter = 0
    while True:
        suffix = "" if counter == 0 else f"-{counter + 1}"
        candidate = result_dir / f"{prefix}{suffix}"
        if candidate.name not in attempted and not candidate.exists():
            return candidate
        counter += 1


def is_windows_access_denied(exc: PermissionError) -> bool:
    """Identify Windows ERROR_ACCESS_DENIED without retrying unrelated errors."""
    return getattr(exc, "winerror", None) == 5 or getattr(exc, "errno", None) == 5
...
def commit_staging_dir(
    ...
    """Commit a complete staging directory without overwriting a prior result.

    The initial existence check in ``unique_package_dir`` cannot by itself prevent
    two renderers from selecting the same name. On Windows, a destination that
    appears between that check and ``rename`` may raise either FileExistsError or
    PermissionError. In that case, move to the next unique fallback name and
    keep the HTML filename aligned with the package directory.
    """
    attempted = {package_dir.name}
    current_html_name = html_name
    retry_count = 0
    while True:
        try:
            staging_dir.rename(package_dir)
            return package_dir, current_html_name
        except FileExistsError:
            next_dir = next_package_dir_after_conflict(result_dir, title, now, attempted)
            next_html_name = f"{next_dir.name}.html"
            (staging_dir / current_html_name).rename(staging_dir / next_html_name)
            attempted.add(next_dir.name)
            package_dir = next_dir
            current_html_name = next_html_name
            retry_count = 0
            ...
            if package_dir.exists():
                ...
                retry_count = 0
                continue
            if retry_count >= len(RENAME_RETRY_DELAYS):
                raise
            time.sleep(RENAME_RETRY_DELAYS[retry_count])
            retry_count += 1
...
def render(manifest_path: Path, result_dir: Path, now: datetime | None = None) -> dict:
    ...
    try:
        ...
        package_dir, html_name = commit_staging_dir(
            staging_dir, package_dir, html_name, result_dir, data["title"], now
        )
    except Exception:
