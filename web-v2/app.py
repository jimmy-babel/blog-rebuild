...
class TaskCreate(BaseModel):
    url: str
    article_mode: Literal["trans", "ori"] = "trans"
    image_mode: Literal["copy", "rebuild", "comic"] = "copy"
    # comic_count is relevant only to comic mode.  Validate it after the mode
    # combination is known so an irrelevant client value cannot reject copy/
    # rebuild jobs that are otherwise ready to run.
    comic_count: int = 5
    ...
    def normalize_brand_aliases(self) -> "TaskCreate":
        ...
        if self.image_mode == "comic" and self.article_mode != "ori":
            raise ValueError("image_mode=comic 必须搭配 article_mode=ori")
        if self.image_mode == "comic" and (isinstance(self.comic_count, bool) or self.comic_count <= 0):
            raise ValueError("comic_count 必须是正整数")
        if self.image_mode != "comic":
            self.comic_count = 5
        return self
    ...
    async with SEMAPHORE:
        ...
        try:
            result = await asyncio.to_thread(
                run_pipeline, task_id, task["url"], task["article_mode"], task["image_mode"],
                task["comic_count"], task["caption_mode"], progress, task["brand_reference"]
            )
            image_failures = int(result.get("image_failures") or 0)
            completion_progress = (
                "处理完成"
                if image_failures == 0
                else f"处理完成（{image_failures} 张配图生成失败，已保留原图链接）"
            )
            DB.update(
                task_id, status="completed", title=result["title"], progress=completion_progress,
...
async def health() -> dict:
    return {
        "ok": True, "database": str(DB.path), "article_fetch": "direct",
...
async def create_task(request: TaskCreate) -> dict:
    ...
    task = DB.create(
        task_id, request.url, request.article_mode, request.image_mode,
        request.comic_count, request.caption_mode, request.brand,
    )
    schedule(task_id)
