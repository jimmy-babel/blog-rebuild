...
CREATE TABLE IF NOT EXISTS tasks (
    id TEXT PRIMARY KEY,
    url TEXT NOT NULL,
    article_mode TEXT NOT NULL DEFAULT 'trans',
    image_mode TEXT NOT NULL,
    comic_count INTEGER NOT NULL DEFAULT 5,
...
class TaskStore:
    ...
    def init(self) -> None:
        with self.lock, closing(self.connect()) as connection, connection:
            ...
            if "article_mode" not in columns:
                connection.execute("ALTER TABLE tasks ADD COLUMN article_mode TEXT NOT NULL DEFAULT 'trans'")
            if "comic_count" not in columns:
                connection.execute("ALTER TABLE tasks ADD COLUMN comic_count INTEGER NOT NULL DEFAULT 5")
    ...
    def create(
        self, task_id: str, url: str, article_mode: str = "trans", image_mode: str = "copy",
        comic_count: int = 5, caption_mode: str = "ai", brand_reference: str = "on",
    ) -> dict:
        created_at = now_iso()
        with self.lock, closing(self.connect()) as connection, connection:
            connection.execute(
                "INSERT INTO tasks (id,url,article_mode,image_mode,comic_count,caption_mode,brand_reference,status,progress,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
                (task_id, url, article_mode, image_mode, comic_count, caption_mode, brand_reference, "queued", "等待执行", created_at),
            )
