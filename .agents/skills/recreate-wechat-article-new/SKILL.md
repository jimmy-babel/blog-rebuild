---
name: recreate-wechat-article-new
description: "Run the new, isolated WeChat Official Account article reconstruction workflow: fetch the article directly from its public page without n8n, clean and rewrite it, recreate retained illustrations with the project's emotion character references, and package a centered offline HTML with its cleaned source archive and image provenance. Use when the user invokes `$recreate-wechat-article-new` with an `https://mp.weixin.qq.com/s/...` URL or explicitly asks for the new workflow. This skill is separate from and does not modify `recreate-wechat-article`."
---

# 微信文章原创重构（新流程）

本目录是 `recreate-wechat-article` 的独立版本。所有脚本、参考资料和配置均由新 skill 自己持有；执行本 skill 时不得回写或替换旧 skill 的文件。
...
```powershell
python .agents/skills/recreate-wechat-article-new/scripts/fetch_article.py "<微信链接>" --work-dir "<工作目录>" --download-images
```
...
```powershell
python .agents/skills/recreate-wechat-article-new/scripts/choose_emotion.py --url "<微信链接>" --index <从1开始的图片序号>
```
...
```powershell
python .agents/skills/recreate-wechat-article-new/scripts/normalize_ai_card.py --input "<工作目录>/raw-cards/image-001.png" --output "<工作目录>/cards/image-001.png"
```
...
```powershell
python .agents/skills/recreate-wechat-article-new/scripts/compose_card.py --illustration "<AI插画>" [--reference "<角色参考图>"] --caption "<新文案>" --emotion <喜|乐|怒|哀|通用> --output "<工作目录>/cards/image-001.png"
```
...
```powershell
python .agents/skills/recreate-wechat-article-new/scripts/build_manifest.py --source "<工作目录>/source.json" --article "<工作目录>/article-plan.json" --output "<工作目录>/manifest.json"
```

构建成功后运行：

```powershell
python .agents/skills/recreate-wechat-article-new/scripts/render_article.py --manifest "<工作目录>/manifest.json" --result-dir result
```
...
```powershell
python .agents/skills/recreate-wechat-article-new/scripts/validate_output.py "<最终HTML路径>" --expected-image-size <ai使用1122x1402|local使用1080x1440>
```
