...
- `articleMode=trans` 时，`text`、`heading`、`quote` 是重新创作后的文章内容；`articleMode=ori` 时正文必须是二次清洗后保留的原文内容。
...
```powershell
python .agents/skills/recreate-wechat-article/scripts/build_manifest.py `
...
- 正文、导语和引用按 `，。！？；,.!;?` 分行并居中；数字中的小数点和千位逗号不拆分。
